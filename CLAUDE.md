...
This checkout is used beside a production Docker deployment on the host serving
`https://coding.colwzq.top`.

- **Do not build on this production host.** Do not run `bun run build`, `next build`, `docker build`,
  or any equivalent production build here. This host can be resource constrained enough for builds to
...
- Current compose override for the app image:
  `/www/compose/claude-code-hub/artifacts/docker-compose.model-market-copy.override.yaml`.
- After loading a remote-built image locally, restart only the app unless explicitly asked otherwise:
  `docker compose -f docker-compose.yaml -f artifacts/docker-compose.model-market-copy.override.yaml up -d --no-deps app`.
