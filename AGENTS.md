...
## Deployment Site Rules for coding.colwzq.top

This checkout is the builder checkout used for the production deployment serving
`https://coding.colwzq.top`.

- Production builds are allowed in this checkout when a build or deploy is required.
- Builder source path: `/home/colwzq/claude-code-hub`.
- Production host: `ubuntu@106.54.22.99`.
- The `/www/compose/claude-code-hub` paths are on the production host, not on the builder checkout.
- Production source path on the production host:
  `/www/compose/claude-code-hub/src-audit`.
- Production compose path on the production host:
  `/www/compose/claude-code-hub`.
- Production app container on the production host:
  `claude-code-hub-app-qzie`.
- Current compose override for the app image on the production host:
  `/www/compose/claude-code-hub/artifacts/docker-compose.model-market-copy.override.yaml`.
- After loading a builder-built image on the production host, restart only the app unless explicitly
  asked otherwise:
  `cd /www/compose/claude-code-hub && docker compose -f docker-compose.yaml -f artifacts/docker-compose.model-market-copy.override.yaml up -d --no-deps app`.
- Do not restart Postgres, Redis, Caddy, MinIO, or audit-worker as part of a normal frontend/backend app deploy.
- Do not print full API keys in logs or final answers. If login regression testing is needed, read keys into
  shell variables and report only `loginType`, redirects, and status codes.
